...
public class WireNetworkCreator
{
	...
	public static GlobalWireNetwork getOrCreateNetwork(Level level)
	{
		...
		return level.getData(ModDataAttachments.WIRE_NETWORK.get());
	}
	...
	private static void replaceOldWireNetwork(Level level)
	{
		...
		if(oldNet==null)
			return;
		// Old net was present: Store it in the new location
		level.setData(ModDataAttachments.WIRE_NETWORK.get(), oldNet);
		// TODO: Remove old file when mixin support is added
